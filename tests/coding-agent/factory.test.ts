...
const oneTurnStream: TestStream = async function* () {
...
test("continueRecent creates a Session at the startup cwd when empty", async () => {
  ...
  try {
    ...
    const project = await createProject({
      keaHome,
      cwd: sub,
      runtime: runtimeFromStream(oneTurnStream),
      modelConfig: model,
    });
...
function twoTurnBashStream(command: string): TestStream {
  let turn = 0;
  const call = {
...
test("project defaults to fail-closed interactions for ask commands", async () => {
  ...
  try {
    const project = await createProject({
      keaHome,
      directory: dir,
      runtime: runtimeFromStream(twoTurnBashStream("rm file.txt")),
      modelConfig: model,
    });
    const harness = await project.createSession();
...
function todoTurnStream(todos: readonly TodoItem[]): TestStream {
  let turn = 0;
  const call = {
    type: "toolCall" as const,
    id: "c1",
...
test("todo content is model-visible while details stay in the in-memory message", async () => {
  ...
  try {
    ...
    const project = await createProject({
      keaHome,
      directory: dir,
      runtime: runtimeFromStream(todoTurnStream(todos)),
      modelConfig: model,
    });
    const harness = await project.createSession();
...
test("todo state recovers from a restored Session after a model switch", async () => {
  ...
  try {
    ...
    const recoveryStream: TestStream = async function* (_model, context) {
    ...
    const secondProject = await createProject({
      keaHome,
      directory: dir,
      runtime: runtimeFromStream(recoveryStream),
      modelConfig: { provider: "test", model: "model-2" },
    });
